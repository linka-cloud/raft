...
func Create(lg *zap.Logger, dirpath string, metadata []byte) (*WAL, error) {
	...
	if err = w.saveCrc(ctx, 0); err != nil {
		return nil, err
	}
	if err = w.encoder.encode(ctx, &walpb.Record{Type: metadataType, Data: metadata}); err != nil {
		return nil, err
	}
	if err = w.SaveSnapshot(ctx, walpb.Snapshot{}); err != nil {
		return nil, err
	}
...
func (w *WAL) cut(ctx context.Context) error {
	ctx, tr := trace.NewTask(ctx, "raft.wal.cut")
	defer tr.End()
	...
	if err = w.saveCrc(ctx, prevCrc); err != nil {
		return err
	}

	if err = w.encoder.encode(ctx, &walpb.Record{Type: metadataType, Data: w.metadata}); err != nil {
		return err
	}

	if err = w.saveState(ctx, &w.state); err != nil {
		return err
	}
...
func (w *WAL) sync(ctx context.Context) error {
	ctx, tr := trace.NewTask(ctx, "raft.wal.sync")
	defer tr.End()
...
func (w *WAL) saveEntry(ctx context.Context, e *raftpb.Entry) error {
	ctx, tr := trace.NewTask(ctx, "raft.wal.saveEntry")
	defer tr.End()

	// TODO: add MustMarshalTo to reduce one allocation.
	b := pbutil.MustMarshal(e)
	rec := &walpb.Record{Type: entryType, Data: b}
	if err := w.encoder.encode(ctx, rec); err != nil {
		return err
	}
	w.enti = e.Index
	return nil
...
func (w *WAL) saveState(ctx context.Context, s *raftpb.HardState) error {
	ctx, tr := trace.NewTask(ctx, "raft.wal.saveState")
	defer tr.End()
	...
	return w.encoder.encode(ctx, rec)
}

func (w *WAL) Save(ctx context.Context, st *raftpb.HardState, ents []raftpb.Entry) error {
	w.mu.Lock()
	defer w.mu.Unlock()
	...
	for i := range ents {
		if err := w.saveEntry(ctx, &ents[i]); err != nil {
			return err
		}
	}
	if err := w.saveState(ctx, st); err != nil {
		return err
	}
...
func (w *WAL) SaveSnapshot(ctx context.Context, e walpb.Snapshot) error {
...
func (w *WAL) saveCrc(ctx context.Context, prevCrc uint32) error {
	return w.encoder.encode(ctx, &walpb.Record{Type: crcType, Crc: prevCrc})
}
