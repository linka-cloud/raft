...
func (m *metaFile) uint64BufAt(info MetaInfo) []byte {
	pos := getOffset(info)
	return m.Data[pos : pos+8]
}
func (m *metaFile) Uint(info MetaInfo) uint64 {
	return binary.BigEndian.Uint64(m.uint64BufAt(info))
}
func (m *metaFile) SetUint(info MetaInfo, id uint64) {
	binary.BigEndian.PutUint64(m.uint64BufAt(info), id)
}

func (m *metaFile) metaBuf() []byte {
	pos := getOffset(Meta)
	return m.Data[pos : pos+512]
}
func (m *metaFile) Meta() []byte {
	return Copy(m.metaBuf())
}

func (m *metaFile) SetMeta(meta []byte) {
	AssertTruef(len(meta) <= 512, "meta exceeds 512 bytes")
	copy(m.metaBuf(), meta)
}

func (m *metaFile) StoreHardState(ctx context.Context, hs *raftpb.HardState) error {
	ctx, tr := trace.NewTask(ctx, "raftwal.wal.StoreHardState")
	defer tr.End()
	...
	AssertTrue(len(buf) < snapshotIndex-hardStateOffset)
...
func (m *metaFile) StoreSnapshot(ctx context.Context, snap *raftpb.Snapshot) error {
	ctx, tr := trace.NewTask(ctx, "raftwal.wal.storeSnapshot")
	defer tr.End()
