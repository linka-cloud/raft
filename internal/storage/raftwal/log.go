...
func openLogFile(dir string, fid int64) (*logFile, error) {
	...
	if err == z.NewFile {
		...
		if keyID != 0 {
			...
			lf.baseIV = Copy(buf[8:])
			AssertTrue(len(lf.baseIV) == 8)
		}
	}
	return lf, nil
}
...
func (lf *logFile) GetRaftEntry(idx int) raftpb.Entry {
	...
	if entry.DataOffset() > 0 {
		AssertTrue(entry.DataOffset() < uint64(len(lf.Data)))
...
func (lf *logFile) generateIV(offset uint64) []byte {
	...
	AssertTrue(copy(iv[:8], lf.baseIV) == 8)
...
func (lf *logFile) bootstrap() error {
	...
	if lf.dataKey, err = lf.registry.LatestDataKey(); err != nil {
		return Wrapf(err, "Error while retrieving datakey in logFile.bootstrap")
	}
	buf := lf.Data[encOffset : encOffset+16]
	...
	if _, err := cryptorand.Read(buf[8:]); err != nil {
		return Wrapf(err, "Error while creating base IV, while creating logfile")
	}
	// Initialize base IV.
	lf.baseIV = Copy(buf[8:])
	AssertTrue(len(lf.baseIV) == 8)
