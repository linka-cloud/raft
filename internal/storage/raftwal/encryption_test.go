...
func TestEntryReadWrite(t *testing.T) {
	...
	require.NoError(t, ds.wal.AddEntries(context.Background(), []raftpb.Entry{{Index: 1, Term: 1, Data: data}}))
...
func TestLogRotate(t *testing.T) {
	...
	for i := 0; i < totalEntries; i++ {
		entry := makeEntry(i)
		require.NoError(t, el.AddEntries(context.Background(), []raftpb.Entry{entry}))
...
func TestLogGrow(t *testing.T) {
	test := func(t *testing.T, key []byte) {
		...
		require.NoError(t, ds.wal.AddEntries(context.Background(), entries))
