...
func TestStorageTerm(t *testing.T) {
	...
	require.NoError(t, ds.reset(context.Background(), ents))
...
func TestStorageLastIndex(t *testing.T) {
	...
	ents := []raftpb.Entry{{Index: 3, Term: 3}, {Index: 4, Term: 4}, {Index: 5, Term: 5}}
	require.NoError(t, ds.reset(ctx, ents))
	...
	require.NoError(t, ds.reset(ctx, []raftpb.Entry{{Index: 6, Term: 5}}))
	last, err = ds.LastIndex()
	if err != nil {
		t.Errorf("err = %v, want nil", err)
	}
...
func TestStorageCreateSnapshot(t *testing.T) {
	...
	for i, tt := range tests {
		require.NoError(t, ds.reset(ctx, ents))
		err := ds.CreateSnapshot(ctx, tt.i, cs, data)
		if err != tt.werr {
			t.Errorf("#%d: err = %v, want %v", i, err, tt.werr)
		}
...
func TestStorageAppend(t *testing.T) {
	...
	for i, tt := range tests {
		require.NoError(t, ds.reset(ctx, ents))
		err := ds.addEntries(ctx, tt.entries)
		if err != tt.werr {
			t.Errorf("#%d: err = %v, want %v", i, err, tt.werr)
		}
...
func TestMetaFile(t *testing.T) {
	...
	require.NoError(t, mf.StoreHardState(ctx, &hs))
	...
	require.NoError(t, mf.StoreSnapshot(ctx, &sp))
...
func TestEntryFile(t *testing.T) {
	...
	require.NoError(t, el.AddEntries(context.Background(), []raftpb.Entry{{Index: 1, Term: 1, Data: []byte("abc")}}))
...
func TestTruncateStorage(t *testing.T) {
	...
	for i := uint64(0); i < numEntries; i++ {
		...
		require.NoError(t, ds.addEntries(ctx, []raftpb.Entry{entry}))
	}
...
func TestStorageOnlySnap(t *testing.T) {
	test := func(t *testing.T, key []byte) {
		...
		require.NoError(t, ds.meta.StoreSnapshot(context.Background(), snap))
	...
	test := func(t *testing.T, key []byte) {
		...
		addEntries := func(start, end uint64) {
			...
			for idx := start; idx <= end; idx++ {
				ent.Index = idx
				require.NoError(t, ds.wal.AddEntries(ctx, []raftpb.Entry{ent}))
				li, err := ds.LastIndex()
				require.NoError(t, err)
		...
		cs := &raftpb.ConfState{}
		require.NoError(t, ds.CreateSnapshot(ctx, N-100, cs, buf))
		fi, err := ds.FirstIndex()
		require.NoError(t, err)
